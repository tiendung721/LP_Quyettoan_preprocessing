...
2. **Xem file bóc tách dữ liệu** — file JSON trong `output` được mở bằng màn hình kiểm
...
File JSON bóc tách là **bộ nhớ tạm của một lô chứng từ**: nó nằm trực tiếp trong
`output`, là bản duy nhất bạn xem/sửa ở Bước 2 và là bản Bước 3 đọc trực tiếp.
Lô mới thay thế lô cũ, và sau khi nhập xong sạch lỗi thì file tạm được dọn đi.
...
```json
{
  ...
  "output_folder": "D:\\RPA_QuyetToan\\output",
  "daily_tracking_file": "D:\\RPA_QuyetToan\\output\\quyet_toan_hang_ngay.xlsx",
...
| `output_folder` | Thư mục duy nhất chứa file JSON bóc tách và file Excel theo dõi |
| `daily_tracking_file` | File theo dõi hàng ngày nằm trong `output` và được cập nhật ở Bước 3 |
| `allowed_extensions` | Các đuôi file được chấp nhận |
| `output_file_patterns` | Mẫu tên file bóc tách cần bắt |
| `file_stable_seconds` | Số giây tối thiểu để coi file output đã ghi xong |
...
## 6. Đặt thư mục lưu file output

Để phần mềm bắt được file output, hãy lưu JSON bóc tách trực tiếp vào thư mục
`output_folder`:

1. Mở Chrome → **Cài đặt** (Settings)
2. Vào **Tải xuống** (Downloads)
3. Ở mục **Vị trí** (Location), bấm **Thay đổi** và chọn:
   `D:\RPA_QuyetToan\output`
...
1. **Bước 1 — Mở trợ lý quyết toán.** Bấm nút (chạy file `.bat`), gửi chứng từ
   lên **GPT Custom** và chờ bóc tách xong, rồi **lưu file JSON vào `output`**.
...
đang dùng dở, hoặc file bóc tách mới nhất trong thư mục `output`.
...
├── output\        File JSON bóc tách và quyet_toan_hang_ngay.xlsx
...
└── app/
    ...
    ├── file_utils.py       Kiểm tra khóa, chờ file ổn định, hash, dọn file JSON tạm
    ├── watcher.py          Theo dõi output bằng watchdog
...
- File JSON bóc tách nằm trong `output` và là **bộ nhớ tạm**, không phải dữ liệu
...
- Có thể tắt/mở lại app bất kỳ lúc nào; phần mềm tự nhận lại file bóc tách đang
  dùng, hoặc file mới nhất trong thư mục `output`.
